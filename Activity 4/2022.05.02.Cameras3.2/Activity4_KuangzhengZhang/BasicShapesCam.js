...
// 	'void main() {\n' +
// 	'	 vec4 a_Position_mod = vec4(a_Position.x * (1.0 + A * sin(B * a_Position.z + C)), a_Position.y * (1.0 + A * sin(B * a_Position.z + C)), a_Position.z, 1.0);\n' +
// 	'  gl_Position = u_ModelMatrix * a_Position_mod;\n' +
// 	'  gl_PointSize = 10.0;\n' +
// 	'  v_Color = a_Color;\n' +
// 	'}\n';
...
var VSHADER_SOURCE =
	...
	'  if (u_Axes == 1) {\n' +
	'    a_Position_new = vec4(a_Position.x, a_Position.y * (1.0 + u_A * sin(u_B * a_Position.x + u_C)), a_Position.z * (1.0 + u_A * sin(u_B * a_Position.x + u_C)), 1.0);\n' +
	'  }\n' +

	'  if (u_Axes == 2) {\n' +
	'    a_Position_new = vec4(a_Position.x * (1.0 + u_A * sin(u_B * a_Position.y + u_C)), a_Position.y, a_Position.z * (1.0 + u_A * sin(u_B * a_Position.y + u_C)), 1.0);\n' +
	'  }\n' +

	'  if (u_Axes == 3) {\n' +
	'    a_Position_new = vec4(a_Position.x * (1.0 + u_A * sin(u_B * a_Position.z + u_C)), a_Position.y * (1.0 + u_A * sin(u_B * a_Position.z + u_C)), a_Position.z, 1.0);\n' +
	'  }\n' +
	// '  vec4 a_Position_new = vec4(a_Position.x * (1.0 + u_A * sin(u_B * a_Position.y + u_C)), a_Position.y, a_Position.z * (1.0 + u_A * sin(u_B * a_Position.y + u_C)), 1.0);\n' +
	'  gl_Position = u_ModelMatrix * a_Position_new;\n' +
	'  gl_PointSize = 10.0;\n' +
	'  v_Color = a_Color;\n' +
	'}\n';
...
function main() {
	...
	var u_A = gl.getUniformLocation(gl.program, 'u_A');
	if (!u_A) {
		console.log('Failed to get the storage location of u_A');
		return;
	}
	var u_B = gl.getUniformLocation(gl.program, 'u_B');
	if (!u_B) {
		console.log('Failed to get the storage location of u_B');
		return;
	}
	var u_C = gl.getUniformLocation(gl.program, 'u_C');
	if (!u_C) {
		console.log('Failed to get the storage location of u_C');
		return;
	}
	var u_Axes = gl.getUniformLocation(gl.program, 'u_Axes');
	if (!u_Axes) {
		console.log('Failed to get the storage location of u_Axes');
		return;
	}
	...
	var tick = function () {
		...
		currentAngle = animate(interval, currentAngle);  // Update the rotation angle
		updatePara(interval);
		drawAll(gl, n, currentAngle, modelMatrix, u_ModelMatrix, u_A, u_B, u_C, u_Axes);   // Draw shapes
...
function drawAll(gl, n, currentAngle, modelMatrix, u_ModelMatrix, u_A, u_B, u_C, u_Axes) {
	//==============================================================================
	...
	*/
	...
	//===========================================================
	//
	if (config.Cylinder.Enable) {
		gl.uniform1f(u_A, config.Cylinder.A.a);
		gl.uniform1f(u_B, config.Cylinder.B.b);
		gl.uniform1f(u_C, config.Cylinder.C.c);
		gl.uniform1i(u_Axes, config.Cylinder.AxesNum);
	}
	...
	gl.uniform1f(u_A, 0.0);
	gl.uniform1f(u_B, 0.0);
	gl.uniform1f(u_C, 0.0);
	gl.uniform1i(u_Axes, 3);

	//===========================================================
	//
	if (config.Sphere.Enable) {
		gl.uniform1f(u_A, config.Sphere.A.a);
		gl.uniform1f(u_B, config.Sphere.B.b);
		gl.uniform1f(u_C, config.Sphere.C.c);
		gl.uniform1i(u_Axes, config.Sphere.AxesNum);
	}
	...
	//===========================================================
	//
	if (config.Torus.Enable) {
		gl.uniform1f(u_A, config.Torus.A.a);
		gl.uniform1f(u_B, config.Torus.B.b);
		gl.uniform1f(u_C, config.Torus.C.c);
		gl.uniform1i(u_Axes, config.Torus.AxesNum);
	}
...
function animate(interval, angle) {
	//==============================================================================
	...
	var newAngle = angle + (ANGLE_STEP * interval) / 1000.0;
	return newAngle %= 360;
}
...
function updatePara(interval) {
	updateABC(interval, name = 'Cylinder');
	updateABC(interval, name = 'Sphere');
	updateABC(interval, name = 'Torus');
}

function updateABC(interval, name) {
	// config[name].A.a += (interval / 1000.0) * config[name].A.speed;
	// config[name].A.a %= 2 * Math.PI;

	// config[name].B.b += (interval / 1000.0) * config[name].B.speed;
	// config[name].B.b %= 2 * Math.PI;

	config[name].C.c += (interval / 1000.0) * config[name].C.speed;
	config[name].C.c %= 2 * Math.PI;
}
...
function initCfg() {
	...
	let Cylinder = gui.addFolder('Cylinder');
	Cylinder.add(config.Cylinder, 'Enable');
	Cylinder.add(config.Cylinder, 'Axes', ['X', 'Y', 'Z']).listen().onChange(() => {
		config.Cylinder.AxesNum = XYZ[config.Cylinder.Axes];
	});
	let CylinderA = Cylinder.addFolder('A');
	CylinderA.add(config.Cylinder.A, 'a', 0, 1).listen();
	// CylinderA.add(config.Cylinder.A, 'speed', 0, 1).listen();
	let CylinderB = Cylinder.addFolder('B');
	CylinderB.add(config.Cylinder.B, 'b', 0, 20).listen();
	// CylinderB.add(config.Cylinder.B, 'speed', 0, 1).listen();
	let CylinderC = Cylinder.addFolder('C');
	// CylinderC.add(config.Cylinder.C, 'c', 0, 2 * Math.PI).listen();
	CylinderC.add(config.Cylinder.C, 'speed', 0, 20).listen();


	let Sphere = gui.addFolder('Sphere');
	Sphere.add(config.Sphere, 'Enable');
	Sphere.add(config.Sphere, 'Axes', ['X', 'Y', 'Z']).listen().onChange(() => {
		config.Sphere.AxesNum = XYZ[config.Sphere.Axes];
	});
	let SphereA = Sphere.addFolder('A');
	SphereA.add(config.Sphere.A, 'a', 0, 1).listen();
	// SphereA.add(config.Sphere.A, 'speed', 0, 1).listen();
	let SphereB = Sphere.addFolder('B');
	SphereB.add(config.Sphere.B, 'b', 0, 20).listen();
	// SphereB.add(config.Sphere.B, 'speed', 0, 1).listen();
	let SphereC = Sphere.addFolder('C');
	// SphereC.add(config.Sphere.C, 'c', 0, 2 * Math.PI).listen();
	SphereC.add(config.Sphere.C, 'speed', 0, 20).listen();

	let Torus = gui.addFolder('Torus');
	Torus.add(config.Torus, 'Enable');
	Torus.add(config.Torus, 'Axes', ['X', 'Y', 'Z']).listen().onChange(() => {
		config.Torus.AxesNum = XYZ[config.Torus.Axes];
	});
	let TorusA = Torus.addFolder('A');
	TorusA.add(config.Torus.A, 'a', 0, 1).listen();
	// TorusA.add(config.Torus.A, 'speed', 0, 1).listen();
	let TorusB = Torus.addFolder('B');
	TorusB.add(config.Torus.B, 'b', 0, 20).listen();
	// TorusB.add(config.Torus.B, 'speed', 0, 1).listen();
	let TorusC = Torus.addFolder('C');
	// TorusC.add(config.Torus.C, 'c', 0, 2 * Math.PI).listen();
	TorusC.add(config.Torus.C, 'speed', 0, 20).listen();
}